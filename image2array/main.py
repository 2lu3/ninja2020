...
def onClickLoadData(event):
    global map_data
    map_data = [
        [[0 for i in range(len(button_labels))] for j in range(image_width)]
            ...
            for wj in range(width):
                for hi_add in range(magnification):
                    for wj_add in range(magnification):
                        map_data[hi * magnification + hi_add][
                            wj * magnification + wj_add
                        ][int(line[hi][wj])] = 1
                ...
                for wj in range(width):
                    for hi_add in range(magnification):
                        for wj_add in range(magnification):
                            map_data[hi * magnification + hi_add][
                                wj * magnification + wj_add
                            ][i + border_area_object + 1] = int(line[hi][wj])
    # pass
...
def onClickOutput(event):
    ...
    output_map = [
        [[0 for i in range(4)] for j in range(output_width)]
...
def onImageMotion(event):
    ...
    canvas.create_rectangle(
        start_x, start_y, moving_x, moving_y, width=1, outline="black", tag="temp"
    )
        ...
        for hi in range(start_y, end_y):
            for wj in range(start_x, end_x):
                if hi < 0 or hi >= image_height or wj < 0 or wj >= image_width:
                    ...
                    map_data[hi][wj][button_labels.index("Yellow")] != 1
                    and map_data[hi][wj][button_labels.index("Wall")] != 1
                    and map_data[hi][wj][button_labels.index("Deposit")] != 1
                ):
                    map_data[hi][wj][paint_option] = 1
    reloadImage()
...
def formatImage():
    ...
    for hi in range(image_height):
        for wj in range(image_width):
            ...
            for i in range(len(button_labels)):
                if (
                    (registered_color[i][0] - image_rgb[hi][wj][0]) ** 2
                    + (registered_color[i][1] - image_rgb[hi][wj][1]) ** 2
                    + (registered_color[i][2] - image_rgb[hi][wj][2]) ** 2
                    <= min_val
                ):
                    min_color_num = i
                    min_val = (
...
def main():
    ...
    canvas = tk.Canvas(
        root,
        width=image_width + margin * 2 + button_margin,
    ...
    line_margin = 2
    canvas.create_rectangle(
        margin - line_margin / 2,
        margin - line_margin / 2,
        image_width + margin + line_margin / 2,
        image_height + margin + line_margin / 2,
        outline="black",
        width=line_margin,
    )
